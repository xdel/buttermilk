...
class SecureSocketWrapper extends Socket 
	 ...
	 * It also sets us as the KeyValidator listener so we will get notified if the key validation fails 
	 * 
	 */
	@Override
	public void autoloadCompleted(AutoloadEvent evt) {
		...
		evt.getHandshake().getValidator().addKeyExchangeListener(this);
	}
