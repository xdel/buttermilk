	...
	public ECKeyContents read() {
		...
		if (iter.hasNext()) {
			...
			if (distUUID.endsWith("-U")) {
				...
				String curveName = String.valueOf(inner.get("CurveName"));
				Encoding enc = Encoding.valueOf(String.valueOf(inner
						.get("Encoding")));
				ECPoint Q = FormatUtil.parseECPoint(curveName, enc,
						String.valueOf(inner.get("Q")));
				BigInteger D = FormatUtil.unwrap(enc,
						String.valueOf(inner.get("D")));
