 ...
 *
 */
public class ArmoredCompressedString implements CharSequence,Serializable {
	...
	public ArmoredCompressedString(byte[] bytes) {
		...
		try {
			data=Base64.encodeBytes(bytes, Base64.URL_SAFE|Base64.GZIP);
		} catch (IOException e) {
	 ...
	 * @return
	 */
	public static ArmoredCompressedString fromString(String unencoded) {
		try {
			byte [] bytes = unencoded.getBytes("UTF-8");
			return new ArmoredCompressedString(bytes);
	...
	public String decode() {
		try {
			return new String(Base64.decode(data, Base64.URL_SAFE|Base64.GZIP), "UTF-8");
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
	...
	public byte [] decodeToBytes() {
		try {
			return Base64.decode(data, Base64.URL_SAFE|Base64.GZIP);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
	...
	public String plainVanillaBase64() {
		try {
			byte [] bytes = Base64.decode(data, Base64.URL_SAFE|Base64.GZIP);
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		ArmoredCompressedString other = (ArmoredCompressedString) obj;
		if (data == null) {
			if (other.data != null)
				return false;
