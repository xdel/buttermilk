 ...
 * Public domain.
 */
import com.cryptoregistry.c2.Curve25519;
...
public class BenchTest {
	...
	private final long time_bench(int bench,int count) {
		...
		for (;count!=0;--count) {
			switch (bench) {
				...
				{
					c2.curve(  e1k, e1,   k);
					c2.curve(e2e1k, e2, e1k);
					c2.curve(  e2k, e2,   k);
					c2.curve(e1e2k, e1, e2k);
					state_update();
					break;
				}
				case BENCH_VERIFY:
				{
					c2.verify(  e1k, e1, c2.ZERO,   k);
					c2.verify(e2e1k, e2, c2.ZERO, e1k);
					c2.verify(  e2k, e2, c2.ZERO,   k);
					c2.verify(e1e2k, e1, c2.ZERO, e2k);
					state_update();
					break;
				}
				case BENCH_KEYGEN:
				{
					c2.keygen(e1k, e1s, e1);
					c2.keygen(e2k, e2s, e2);
					c2.verify(e1e2k, e1s, k, e1k);
					c2.verify(e2e1k, e2s, k, e2k);
					state_update();
					break;
				}
				case BENCH_SIGN:
				{
					c2.keygen(e1k, e1, e1);
					c2.keygen(e1e2k, null, e2);
					c2.sign(e2k, e1e2k, e2, e1);
					c2.verify(e2e1k, e2k, e1e2k, e1k);
					state_update();
					break;
				}
			}
	...
	double benchmark(String what, int bench,int div,double offset) {
		final int TRIES=3;
